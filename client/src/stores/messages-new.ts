...
export const useMessagesStore = defineStore('messages', () => {
  ...
  async function sendMessage(content: string, userId: number, chatId?: number, isThinking: boolean = false) {
    ...
    try {
      ...
      socketService.on(ServerWebSocketEvent.ChatId, (data: { chatId: number }) => {
      ...
      socketService.on(ServerWebSocketEvent.MessageChunk, (data: { content: string; chatId: number }) => {
      ...
      socketService.on(ServerWebSocketEvent.Reasoning, (data: { content: string; chatId: number }) => {
        if (!streamingMessages.value.has(data.chatId)) {
          initializeStreamingMessage(data.chatId);
        }
      ...
      socketService.on(ServerWebSocketEvent.Title, (data: { title: string; chatId: number }) => {
        if (!streamingMessages.value.has(data.chatId)) {
          initializeStreamingMessage(data.chatId);
        }
      ...
      socketService.on(ServerWebSocketEvent.ToolCall, (data: { toolCall: any; chatId: number }) => {
        if (!streamingMessages.value.has(data.chatId)) {
          initializeStreamingMessage(data.chatId);
        }
      ...
      socketService.on(ServerWebSocketEvent.ToolResult, (data: { toolName: string; result: any; isError?: boolean; chatId: number }) => {
        if (!streamingMessages.value.has(data.chatId)) {
          initializeStreamingMessage(data.chatId);
        }
      ...
      socketService.on(ServerWebSocketEvent.MessageComplete, (data: { aiMessage: Message; chatId: number }) => {
      ...
      socketService.on(ServerWebSocketEvent.Error, (data: { error: string; chatId?: number }) => {
