  ...
  <div class="mcp-list">
    ...
    <div class="mcp-list-content">
      <n-empty v-if="servers.length === 0" description="No MCP servers configured">
        <template #extra>
          <n-button size="small" @click="$emit('add-server')">
      ...
      <n-space v-else vertical :size="12">
        <MCPItem v-for="server in servers" :key="server.id" :server="server" @edit="$emit('edit-server', $event)"
          @delete="$emit('delete-server', $event)" @toggle="$emit('toggle-server', $event)"
          @connect="$emit('connect-server', $event)" @disconnect="$emit('disconnect-server', $event)" />
      </n-space>
    </div>
      ...
      <n-space vertical :size="16">
        <div class="setting-row">
          <n-form-item label="Debug Logging" label-placement="left">
            <n-switch :value="globalConfig.enableDebugLogging"
              @update:value="updateGlobalConfig('enableDebugLogging', $event)" />
          </n-form-item>
          <div class="setting-help">
            Enable detailed logging for MCP server connections
          </div>
        </div>

        <div class="setting-row">
          <n-form-item label="Default Timeout" label-placement="left">
            <n-input-number :value="globalConfig.defaultTimeout"
              @update:value="updateGlobalConfig('defaultTimeout', $event)" :min="1000" :max="30000" :step="1000"
              style="min-width: 150px" />
            <template #suffix>ms</template>
          </n-form-item>
          <div class="setting-help">
            Default connection timeout for MCP servers (1-30 seconds)
          </div>
        </div>

        <div class="setting-row">
          <n-form-item label="Max Concurrent Connections" label-placement="left">
            <n-input-number :value="globalConfig.maxConcurrentConnections"
              @update:value="updateGlobalConfig('maxConcurrentConnections', $event)" :min="1" :max="20" :step="1"
              style="min-width: 150px" />
          </n-form-item>
          <div class="setting-help">
            Maximum number of simultaneous MCP server connections
          </div>
        </div>

        <div class="setting-row">
          <n-form-item label="Auto Discovery" label-placement="left">
            <n-switch :value="globalConfig.autoDiscovery" @update:value="updateGlobalConfig('autoDiscovery', $event)" />
          </n-form-item>
          <div class="setting-help">
            Automatically discover MCP servers in common locations
          </div>
        </div>
      </n-space>
    </div>
...
import { NSpace, NButton, NIcon, NEmpty, NDivider, NFormItem, NSwitch, NInputNumber } from 'naive-ui'
